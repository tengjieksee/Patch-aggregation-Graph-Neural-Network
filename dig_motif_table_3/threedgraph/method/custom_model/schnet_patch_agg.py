...
class update_v(torch.nn.Module):
    ...
    def forward(self, v, e, edge_index):
        ...
        print(self.var_effective_patch_dim,self.var_effective_patch_dim*self.var_num_patches)
        ...
        #e2 = self.fc_pre_1(e2)
        if self.slot_0 == "e1":
            e1_0 = e.reshape(-1, self.var_num_patches, self.var_effective_patch_dim)#(-1,4,32)
        elif self.slot_0 == "e2":
            e1_0 = e.reshape(-1, self.var_num_patches, self.var_effective_patch_dim)#(-1,4,32)
        
        if self.slot_1 == "e1":
            e2_0 = e.reshape(-1, self.var_num_patches, self.var_effective_patch_dim)#(-1,4,32)
        elif self.slot_1 == "e2":
            e2_0 = e.reshape(-1, self.var_num_patches, self.var_effective_patch_dim)#(-1,4,32)
...
class Custom_Model(torch.nn.Module):
    ...
    """
    def __init__(self, device = "cuda:0", energy_and_force=False, cutoff=10.0, num_layers=6, hidden_channels=128, out_channels=1, num_filters=128, num_gaussians=50, var_effective_patch_dim = 16, var_num_patches = 8, var_total_feature_dim=128, slot_0 = "e1", slot_1 = "e2"):
    ...
    def forward(self, batch_data):
        ...
        opt_aux = True
        return (u,aux_loss_all,opt_aux,node_sim_loss,0.)#return (u,aux_loss_all,opt_aux,node_sim_loss)
