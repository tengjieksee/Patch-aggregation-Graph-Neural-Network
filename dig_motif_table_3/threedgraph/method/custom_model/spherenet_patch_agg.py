...
class update_v(torch.nn.Module):
    def __init__(self, hidden_channels, out_emb_channels, out_channels, num_output_layers, act, output_init, var_effective_patch_dim, var_num_patches, var_total_feature_dim):
    ...
    def forward(self, e, i, batch_):
        ...
        v = self.lin(v)
        return v #(v, euclidean_distance_matrix)
...
class update_u(torch.nn.Module):
    ...
    def forward(self, u, v, batch):
        u += scatter(v.cuda(), batch.cuda(), dim=0)
        return u
...
class Custom_Model(torch.nn.Module):
    ...
    def __init__(
        ...
        act=swish, output_init='GlorotOrthogonal', use_node_features=True, var_effective_patch_dim = 32, var_num_patches = 4, var_total_feature_dim=128):
        ...
        self.init_v = update_v(hidden_channels, out_emb_channels, out_channels, num_output_layers, act, output_init, var_effective_patch_dim, var_num_patches, var_total_feature_dim)
        self.init_u = update_u()
        self.emb = emb(num_spherical, num_radial, self.cutoff, envelope_exponent)

        self.update_vs = torch.nn.ModuleList([
            update_v(hidden_channels, out_emb_channels, out_channels, num_output_layers, act, output_init, var_effective_patch_dim, var_num_patches, var_total_feature_dim) for _ in range(num_layers)])
    ...
    def forward(self, batch_data):
        ...
        v = self.init_v(e, i, batch_data.batch)
        ...
        for update_e, update_v, update_u in zip(self.update_es, self.update_vs, self.update_us):
            ...
            v = update_v(e, i, batch_data.batch)
            u = update_u(u, v, batch) #u += scatter(v, batch, dim=0)
        u = u
        euclidean_distance_matrix = 69.#euclidean_distance_matrix / (4+1)
