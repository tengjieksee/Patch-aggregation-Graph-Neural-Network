...
class TensorEmbedding(nn.Module):
    ...
    def forward(
        ...
        #print(Iij.shape)#9000,64,3,3
        #print(Aij.shape)#9000,64,3,3
        #print(Sij.shape)#9000,64,3,3
        #time.sleep(999)
...
class aggregation_custom_backup(torch.nn.Module):
    def __init__(self):
        super(aggregation_custom_backup, self).__init__()
...
class aggregation_custom(torch.nn.Module):
    def __init__(self, hidden_channels):
        ...
        self.fc_all = torch.nn.Linear(int(self.factor*3*3/2),self.factor*3*3,bias=False)
        #16 for 256 total dim
        #4 for 64 total dim
        self.learnable_param = nn.Parameter(torch.rand(1,128,3,3)*0.00001)#0.01
    ...
    def forward(self, edge_index, factor, tensor, natoms):
        ##print("MPNN")
        
        msg0 = factor * tensor.index_select(0, edge_index[1])
        shape0 = (natoms, tensor.shape[1], tensor.shape[2], tensor.shape[3])
        tensor_m0 = torch.zeros(*shape0, device=tensor.device, dtype=tensor.dtype)
        tensor_m0 = tensor_m0.index_add(0, edge_index[0], msg0)
        
        
        msg = factor * tensor.index_select(0, edge_index[1])#256 to 64 which is by factor of 4
        
        #print(msg.shape)#9954,256,3,3
        #9954, 64, 3, 3
        
        
        msg = msg.reshape(-1,self.parts,self.factor,3,3).reshape(-1,self.parts,self.factor*3*3).reshape(-1,self.factor*3*3)
        
        #print(msg.shape)#159264, 36
        #time.sleep(999)
        ...
        var_num_patches = 2
        var_effective_patch_dim = int(self.factor*3*3/2)
        
        e = msg#torch.permute(msg, (0,2,3,1))
        #e = torch.reshape(e, (-1, self.factor*3*3))
        ##print(msg.shape)#[9954, 256, 3, 3]
        ...
        for _i in range(len(e1_lst)):
            ...
            out_lst = torch.cat((out_lst,out_),-1)
        out_lst = out_lst[:,1:]#+e2.to(self.device)#num_edges,128
        out = torch.reshape(out_lst,(-1,self.parts*self.factor,3,3))
        #print(out.shape)#[39816, 16, 3, 3]
        
        out = torch.reshape(out,(-1,self.parts*self.factor*var_num_patches,3,3))
        #out = torch.reshape(out,(-1,self.parts*self.factor,3,3))
        
        #print(out.shape)
        
        tensor_m = torch.zeros(*shape, device=tensor.device, dtype=tensor.dtype)
        
        #index_add_(): Number of indices (9954) should be equal to source.size(dim): (19908), for dim: 0
        #factor 2
        
        #print(out.shape)#19908, 64, 3, 3
        
        y_m1 = tensor_m.index_add(0, edge_index[0], out)
        
        out = (torch.abs(self.learnable_param)*y_m1) + tensor_m0 #index_add_(): Number of indices (9954) should be equal to source.size(dim): (19908), for dim: 0
        #print(f"{torch.abs(self.learnable_param).item()}, {torch.max(y_m1).item()}, {torch.max(tensor_m0).item()}")
        ##print(out.shape)
        #out = scatter(out, i, dim=0)
        ##print(out.shape)
        return out
...
class Interaction(nn.Module):
    ...
    def __init__(
        ...
        self.tensor_message_passing_0 = aggregation_custom(64)
        #self.tensor_message_passing_1 = aggregation_custom(64)
        #self.tensor_message_passing_2 = aggregation_custom(64)
    ...
    def forward(
        ...
        Y = I + A + S
        #Im = self.tensor_message_passing_0(        
