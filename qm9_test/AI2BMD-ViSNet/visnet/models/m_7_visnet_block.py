...
def print_details(name_, out):
    #print(f"{name_}: {torch.min(out).item()}, {torch.mean(out).item()}, {torch.max(out).item()}, {torch.std(out).item()}")
    pass
...
class aggregation_custom_backup(torch.nn.Module):
    def __init__(self):
        ...
        #self.ln0 = torch.nn.LayerNorm(128)
    ...
    def forward(self, x, index, dim, dim_size):
        ...
        print_details("ex_x after f_all", ex_x)
        #y = 5x-2
        #-1 to 1 for x
        ...
        e1_1 = torch.reshape(e1_1,(-1, var_num_patches, var_num_patches, var_effective_patch_dim))
        e2_0 = e2_0.unsqueeze(2).expand(-1,-1,var_num_patches,-1)#-1,4,4,32
        e2_0 = torch.permute(e2_0,(0,2,1,3))
        
        print_details("e1_1", e1_1)
        print_details("e2_0", e2_0)
        
        y_ = e1_1 * e2_0
        
        print_details("y_ = e1_1 * e2_0", y_)
        
        out = torch.sum(y_,1)#replace o_proj
        out = torch.reshape(out,(-1,128))# + msg
        
        #out_1 = msg * self.act(out)
        
        
        #out = scatter(out_1, index, dim=0)
        print_details("out = scatter(out, index, dim=0)", out)
        
        #out = scatter(msg, index, dim=0)
        #print_details("out = scatter(msg, index, dim=0)", out)
        
        
        #loss_tensor = torch.zeros_like(out)[:,0:1]# + loss_aux_total
        #out = torch.cat((out, loss_tensor),-1)
...
class aggregation_custom(torch.nn.Module):
    def __init__(self):
        ...
        self.fc_all = torch.nn.Sequential(torch.nn.Linear(int(128/2),128))
...
class ViS_MP(MessagePassing):
    ...
    def reset_parameters(self):
        ...
        #nn.init.xavier_uniform_(self.v_1_proj.weight)
        #self.v_1_proj.bias.data.fill_(0)
    ...
    def forward(self, x, vec, edge_index, r_ij, f_ij, d_ij):
        print_details("begin x", x)
        x = self.layernorm(x)
        print_details("ln x", x)
        ...
        #print(k.shape)        
        #v = self.aggregation_method(x, index, dim=self.node_dim, dim_size=dim_size)
    ...
    def message(self, q_i, k_j, v_j, vec_j, dk, dv, r_ij, d_ij):
        ...
        print_details("attn", attn)
        
        v_j = v_j * dv
        
        print_details("v_j = v_j * dv", v_j)
        
        v_j = (v_j * attn.unsqueeze(2)).view(-1, self.hidden_channels)
        
        print_details("v_j = (v_j * attn.unsqueeze(2)).view(-1, self.hidden_channels)", v_j)
