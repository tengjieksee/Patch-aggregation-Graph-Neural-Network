...
class aggregation_custom(torch.nn.Module):
    ...
    def forward(self, x, index, dim, dim_size):
        ...
        #out = msg + out
        #print(out)
        #print(torch.min(out),torch.mean(out),torch.max(out))#tensor(-0.0002, device='cuda:0') tensor(1.8510e-06, device='cuda:0') tensor(0.0046, device='cuda:0')
        
        
        #time.sleep(999)
        
        #out = self.act(out)#destroy info
        #out = msg * self.act(out)
