...
def analyze_frequency_components(latent_vector):
    # Compute FFT
    latent_vector = latent_vector.cpu().detach().numpy().flatten()
    fft_values = np.fft.fft(latent_vector)
    fft_freqs = np.fft.fftfreq(len(latent_vector))
    ...
    high_pass_mask = np.abs(fft_freqs) > cutoff_freq

    low_pass_fft = fft_values * low_pass_mask
    high_pass_fft = fft_values * high_pass_mask

    # Compute inverse FFT to get the filtered signals
    low_pass_signal = np.fft.ifft(low_pass_fft).real
    high_pass_signal = np.fft.ifft(high_pass_fft).real

    # Compute total energy in low-pass and high-pass components
    low_pass_energy = np.sum(np.abs(low_pass_signal) ** 2)
    high_pass_energy = np.sum(np.abs(high_pass_signal) ** 2)

    return magnitude, low_pass_signal, high_pass_signal, low_pass_energy, high_pass_energy
...
class aggregation_custom(torch.nn.Module):
    def __init__(self):
        ...
        self.learnable_param = nn.Parameter(torch.zeros(1,128)+1.)#self.learnable_param = nn.Parameter(torch.zeros(1,128)+0.00001)#0.01
        self.layernorm = nn.LayerNorm(64)
        
        #self.fc_lin = torch.nn.Linear(128,128,bias=False)
    ...
    def forward(self, x, index, dim, dim_size):
        ...
        #torch.sigmoid(input)
        #print("before")
        #print(torch.min(e1_0).item(), torch.mean(e1_0).item(), torch.max(e1_0).item(), torch.std(e1_0).item())
        e1_0 = self.layernorm(e1_0)
        #print(torch.min(e1_0).item(), torch.mean(e1_0).item(), torch.max(e1_0).item(), torch.std(e1_0).item())
        ...
        print(f"{torch.abs(torch.mean(self.learnable_param)).item()}, {torch.max(out).item()}, {torch.max(out_original).item()}")
        ##print(out)
        ##print(torch.min(out),torch.mean(out),torch.max(out),torch.std(out))
        
        magnitude, low_pass_signal, high_pass_signal, low_pass_energy, high_pass_energy = analyze_frequency_components(out_total)
        #print(magnitude)
        print(low_pass_energy)#513.4958278920888
        print(high_pass_energy)
        
        
        #magnitude, low_pass_signal, high_pass_signal, low_pass_energy, high_pass_energy = analyze_frequency_components(out)
        #print(magnitude)
        #print(low_pass_energy)#166.5467149963904
        #print(high_pass_energy)
        
        
        magnitude, low_pass_signal, high_pass_signal, low_pass_energy, high_pass_energy = analyze_frequency_components(out_original)
        #print(magnitude)
        print(low_pass_energy)#513.4933949130651
        print(high_pass_energy)
        
        
        #lin_msg = self.fc_lin(msg)
        #magnitude, low_pass_signal, high_pass_signal, low_pass_energy, high_pass_energy = analyze_frequency_components(lin_msg)
        #print(magnitude)
        #print(low_pass_energy)#156.94075179306142
        #print(high_pass_energy)
