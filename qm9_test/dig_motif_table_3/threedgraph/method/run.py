...
# Example usage:
#csv_logger = CSVLogger('training_log.csv', fieldnames=['epoch', 'loss', 'accuracy'])

# Simulate training loop
#for epoch in range(1, 11):
#    # Replace these with your actual training metrics
#    current_loss = 0.1 * epoch
#    current_accuracy = 1.0 - 0.05 * epoch
#
#    # Log metrics
#    csv_logger.log({'epoch': epoch, 'loss': current_loss, 'accuracy': current_accuracy})
...
class run():
    ...
    def run(self, device, train_dataset, valid_dataset, test_dataset, model, loss_func, evaluation, epochs=500, batch_size=32, vt_batch_size=32, lr=0.0005, lr_decay_factor=0.5, lr_decay_step_size=50, weight_decay=0, 
        ...
        #wandb.log({"#seed": seed_id_for_record})
        ...
        warmup_steps = 3000#total_training_steps * warmup_fraction
        ...
        for epoch in range(1, epochs + 1):
            ...
            #wandb.log({"#Epoch - "+str(name_run): float(epoch)})
            print('\nTraining...', flush=True)
            ...
            train_mae, unit_name = self.scale_the_metrics_(train_mae,name_run)
            
            ema.update()
            
            ###Normal weights
            print('\n\nEvaluating...', flush=True)
            valid_mae = self.val(model, valid_loader, energy_and_force, p, evaluation, device, mean_train , std_train)
            valid_mae, unit_name = self.scale_the_metrics_(valid_mae,name_run)

            print('\n\nTesting...', flush=True)
            test_mae = self.val(model, test_loader, energy_and_force, p, evaluation, device, mean_train , std_train)
            test_mae, unit_name = self.scale_the_metrics_(test_mae,name_run)
            ...
            #    valid_mae_EMA, unit_name = self.scale_the_metrics_(valid_mae_EMA,name_run)
            #    
            #    print('\n\nTesting_EMA...', flush=True)
            #    test_mae_EMA = self.val(model, test_loader, energy_and_force, p, evaluation, device, mean_train , std_train)
            #    test_mae_EMA, unit_name = self.scale_the_metrics_(test_mae_EMA,name_run)
            
            
            
            
            print()
            #print({'Train': train_mae, 'Validation': valid_mae, 'Test': test_mae, 'Validation_EMA': valid_mae, 'Test_EMA': test_mae_EMA})
            ...
            if log_dir != '':
                ...
                #writer.add_scalar('valid_mae_EMA - '+str(unit_name), valid_mae_EMA, epoch)
                #writer.add_scalar('test_mae_EMA - '+str(unit_name), test_mae_EMA, epoch)
                #wandb.log({'train_mae - '+str(unit_name): train_mae})
                #wandb.log({'valid_mae - '+str(unit_name): valid_mae})
                #wandb.log({'test_mae - '+str(unit_name): test_mae})
                #csv_logger.log({'epoch': epoch, 'training_MAE - '+str(unit_name): train_mae, 'validation_MAE - '+str(unit_name): valid_mae, 'test_MAE - '+str(unit_name): test_mae, "Params": num_params, "Seed": seed_id_for_record})
                csv_logger.log([epoch, train_mae, valid_mae, test_mae, num_params, seed_id_for_record])
                #logger.log([epoch, current_loss, current_accuracy])
                
                #wandb.log({'valid_mae_EMA - '+str(unit_name): valid_mae_EMA})
                #wandb.log({'test_mae_EMA - '+str(unit_name): test_mae_EMA})
            ...
            #        #checkpoint = {'epoch': epoch, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'scheduler_state_dict': scheduler.state_dict(), 'best_valid_mae': best_valid, 'num_params': num_params}
            #        ###checkpoint = {'epoch': epoch, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'best_valid_mae': best_valid, 'num_params': num_params}
            #        #torch.save(checkpoint, os.path.josin(save_dir, 'valid_checkpoint_epoch_'+str(epoch)+'.pt'))
            #        pass
        ...
        #wandb.log({'Best validation MAE so far': best_valid})
        print(f'Test MAE when got best validation result: {best_test}')
        #wandb.log({'Test MAE when got best validation result': best_test})
        #csv_logger.close()
    ...
    def train(self, model, optimizer, train_loader, energy_and_force, p, loss_func, device, mean_train , std_train, epoch, scheduler):
        ...
        for step, batch_data in enumerate(tqdm(train_loader)):
            ...
            out, aux_loss, opt_aux, recon_loss = model(batch_data)
            ...
            if energy_and_force:
                ...
                #time.sleep(999)
                loss_inner = loss_func(out, batch_data.y.unsqueeze(1)) + aux_loss
                loss = loss_func(out, batch_data.y.unsqueeze(1))# + aux_loss
                #print(loss)
                #print(batch_data.y)
                #time.sleep(999)
                if opt_aux == True:
                    #wandb.log({'Training loss per step': loss, 'Aux loss per step': aux_loss, 'recon_loss':recon_loss})
                    #wandb.log({'Training loss per step': loss, 'Aux loss per step': aux_loss, 'Recon_loss':recon_loss, 'Dist_loss':dist_loss, 'Dist_loss_real':dist_loss_real})
                    #wandb.log({'Training loss per step': loss, 'Aux loss per step': aux_loss, 'node_sim_loss': node_sim_loss})
                    #wandb.log({'Training loss per step': loss, 'Aux loss per step': aux_loss})
                    pass
                else:
                    #wandb.log({'Training loss per step': loss})
                    pass
            ...
            if epoch == 1 and step <= 5000:
                for g in optimizer.param_groups:
                    #g['lr'] = g['lr']+2.5e-06#3e-07
                    #wandb.log({'learning rate': g['lr']})
                    pass
            else:
                for g in optimizer.param_groups:
    ...
    def val(self, model, data_loader, energy_and_force, p, evaluation, device, mean_train , std_train):
        r"""
        ...
        for step, batch_data in enumerate(tqdm(data_loader)):
            batch_data = batch_data.to(device)
            out, aux_loss, opt_aux, _ = model(batch_data)
            if energy_and_force:
                force = -grad(outputs=out, inputs=batch_data.pos, grad_outputs=torch.ones_like(out),create_graph=True,retain_graph=True)[0]
            ...
            targets = torch.cat([targets, sec_t], dim=0)#targets = torch.cat([targets.to("cpu"), batch_data.y.unsqueeze(1).to("cpu")], dim=0)
