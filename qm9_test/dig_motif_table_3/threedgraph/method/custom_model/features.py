...
class dist_emb(torch.nn.Module):#RBF function
    ...
    def forward(self, dist):
        ...
        #print(dist.shape)#[8588, 1]
        #print(dist)#[71400, 1]
        term_0 = self.envelope(dist)
        freq_ = self.freq
        term_1 = (freq_ * dist)
        term_2 = term_1.sin()
        total =  term_0 * term_2
        #print(term_0.shape)
        #print(freq_)#tensor([ 3.1416,  6.2832,  9.4248, 12.5664, 15.7080, 18.8496]
        #print(term_1.shape)
        #print(term_2.shape)
        
        #print(total.shape)#[8588, 6]
        #time.sleep(999)
        return total


class dist_emb_Transformer(torch.nn.Module):#RBF function
    def __init__(self, num_radial, cutoff=5.0, envelope_exponent=5):
        super(dist_emb_Transformer, self).__init__()
        self.cutoff = cutoff
        self.envelope = Envelope(envelope_exponent)
    ...
    def forward(self, pre_dist):
        #print("RBF")
        #print(pre_dist.shape)
        dist = torch.flatten(pre_dist).unsqueeze(-1) / self.cutoff
        #print(dist.shape)
        #print(dist)#[71400, 1]
        term_0 = self.envelope(dist)
        freq_ = self.freq
        ...
        #print(term_1.shape)
        #print(term_2.shape)
        total = torch.reshape(total,(-1,pre_dist.shape[1],pre_dist.shape[2]*self.num_radial))
        return total

class angle_emb(torch.nn.Module):#SBF function
